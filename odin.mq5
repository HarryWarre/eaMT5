...
input int    InpMagic           = 202602;  // Magic Number
...
void OnTick()
  {
   ...
   // Quét 13 tín hiệu PA
   int signal = CheckPASignalExtended();
   if(signal != 0)
      ExecuteEntry(signal);
  }

//+------------------------------------------------------------------+
//| SIGNAL: RSI + D1 + 13 PA Models                                 |
//+------------------------------------------------------------------+
int CheckPASignalExtended()
  {
   // 1. D1 Trend
   ...
   // 3. Price Action Check (Cần 6 nến cho mẫu hình 5 nến + shift)
   MqlRates rates[];
...
void ManageMartingale() {
   ...
   if(isAgainst && dist >= InpMartGridPips) {
      ...
      if(res) Print("Odin: Bồi đòn! Tầng=", martingaleLevel, " Lot=", lot);
   }
}
