...
private:
   ...
   bool              m_useRiskScaling;   // New: Toggle for Auto-Scaling
   double            m_scalingBase;      // Capital Divider for Scaling
...
public:
                     CAgent(void);
                    ~CAgent(void);
   bool              Init(string symbol, int magic, double gridStep, bool isPercentage, double lotSize, int maxGrid, bool useRiskScaling, double scalingBase, COrderDispatcher *dispatcher);
...
bool CAgent::Init(string symbol, int magic, double gridStep, bool isPercentage, double lotSize, int maxGrid, bool useRiskScaling, double scalingBase, COrderDispatcher *dispatcher)
  {
   m_symbol    = symbol;
   m_magic     = magic;
   m_gridStep  = gridStep;
   ...
   m_maxGrid   = maxGrid;
   m_useRiskScaling = useRiskScaling;
   m_scalingBase = (scalingBase > 0) ? scalingBase : 1000.0;
...
void CAgent::RecalculateLotSize()
  {
   ...
   if(!m_useRiskScaling)
     {
      // Fixed Lot Mode: Always use the fixed input
      m_currentLotSize = m_fixedLotSize;
      return; 
     }
     
   double divisor = (m_scalingBase > 0) ? m_scalingBase : 1000.0;
   double normalized = m_allocatedCapital / divisor;
...
private:
   ...
   bool              m_useRiskScaling;
   
public:
                     CCore(void);
                    ~CCore(void);
   bool              Init(string setupCSV, int magicBase, int mlPeriod, double defaultLot, int maxGrid, double minMargin, int newsBuffer, bool useRiskScaling, double manualBase);
   void              OnTick();
   void              OnDeinit();
private:
   void              ParseSetup();
...
CCore::CCore(void) : m_agentCount(0), m_totalCapital(0), m_minMarginLevel(500.0), m_useRiskScaling(false) 
  {
...
bool CCore::Init(string setupCSV, int magicBase, int mlPeriod, double defaultLot, int maxGrid, double minMargin, int newsBuffer, bool useRiskScaling, double manualBase)
  {
   m_setupCSV = setupCSV;
   ...
   // --- SCALING BASE LOGIC ---
   double resolvedBase = 1000.0; // Fallback
   if(m_useRiskScaling)
     {
      if(manualBase > 0)
        {
         resolvedBase = manualBase;
         Print("Core Example: Scaling Base set MANUALLY to ", resolvedBase);
        }
      else
        {
         // Auto-Detect with Persistence
         string gvName = "Chimera_Base_" + IntegerToString(m_magicBase);
         
         if(GlobalVariableCheck(gvName))
           {
            resolvedBase = GlobalVariableGet(gvName);
            Print("Core Example: Scaling Base LOADED from Memory: ", resolvedBase);
           }
         else
           {
            resolvedBase = AccountInfoDouble(ACCOUNT_EQUITY);
            if(resolvedBase <= 0) resolvedBase = 1000.0; // Safety
            GlobalVariableSet(gvName, resolvedBase);
            Print("Core Example: Scaling Base DETECTED & SAVED: ", resolvedBase);
           }
        }
     }
   ...
   for(int i=0; i<m_agentCount; i++)
     {
      ...
      m_agents[i].Init(m_configs[i].symbol, magicBase + i, m_configs[i].gridStep, m_configs[i].isPercentage, finalLot, maxGrid, m_useRiskScaling, resolvedBase, m_dispatcher);
...
void CCore::OnTick()
  {
   ...
   // --- REAL-TIME DASHBOARD ---
   string gvName = "Chimera_Base_" + IntegerToString(m_magicBase);
   double scalingBase = GlobalVariableCheck(gvName) ? GlobalVariableGet(gvName) : 1000.0;
   double ratio = (scalingBase > 0) ? AccountInfoDouble(ACCOUNT_EQUITY) / scalingBase : 1.0;

   string comm = "=== CHIMERA ENTERPRISE REAL-TIME DASHBOARD ===\n";
   comm += "------------------------------------------------------------\n";
   comm += "Time: " + TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES) + "\n";
   ...
   if(m_useRiskScaling)
     {
      comm += "Scaling Base: $" + DoubleToString(scalingBase, 2) + " (Mult: " + DoubleToString(ratio, 2) + "x)\n";
     }
   else
     {
      comm += "Scaling: DISABLED (Fixed Lots)\n";
     }
   
   comm += "------------------------------------------------------------\n";
   comm += StringFormat("%-3s | %-7s | %-10s | %-6s\n", "ID", "Symbol", "Net Profit", "ROI");
   comm += "------------------------------------------------------------\n";
   ...
   for(int i=0; i<m_agentCount; i++)
     {
      ...
      comm += StringFormat("%-3d | %-7s | %-10.2f | %-5.2f%%\n", 
                           i, m_agents[i].GetSymbol(), profit, roi);
     }
   comm += "------------------------------------------------------------\n";
...
// Daily Report functionality removed. Logic replaced by Real-Time Dashboard in OnTick.
...
void CCore::ParseSetup()
  {
   ...
   // Format: Symbol:Step[:Lot] (e.g. EURUSD:30, XAUUSD:50:0.01)
   string result[];
   int k = StringSplit(m_setupCSV, ',', result);
   
   m_agentCount = k;
   if(k > 0) ArrayResize(m_configs, k);
   
   for(int i=0; i<k; i++)
     {
      string parts[];
      int p = StringSplit(result[i], ':', parts);
      
      // Defaults
      m_configs[i].symbol = (p > 0) ? parts[0] : "UNK";
      m_configs[i].gridStep = 30.0;
      m_configs[i].baseLot = 0.0; // Use Default
      m_configs[i].isPercentage = false;
      
      // Parse Step (e.g. "30" or "0.5%")
      if(p >= 2)
        {
         string s = parts[1];
         StringTrimLeft(s); StringTrimRight(s);
         m_configs[i].isPercentage = (StringFind(s, "%") >= 0);
         StringReplace(s, "%", "");
         m_configs[i].gridStep = StringToDouble(s);
        }
      
      // Parse Lot override
      if(p >= 3) m_configs[i].baseLot = StringToDouble(parts[2]);
      
      // Trim Symbol
      StringTrimLeft(m_configs[i].symbol); StringTrimRight(m_configs[i].symbol);
     }
  }
...
input string   InpAgentSetup     = "EURUSD:30:0.1,XAUUSD:50:0.01"; // Setup (Sym:Step[:Lot])
...
input bool     InpUseRiskScaling     = true;     // Enable Auto Volume Scaling (Compounding)
input double   InpManualBaseCapital  = 0.0;      // [Optional] Manually set Base Capital (0 = Auto-Detect)
 ...
 int OnInit()
   {
    ...
    g_manager = new CCore();
    if(!g_manager.Init(InpAgentSetup, InpMagicBase, InpMLPeriod, InpDefaultLot, InpMaxGrid, InpMinMargin, InpNewsBuffer, InpUseRiskScaling, InpManualBaseCapital))
      {
